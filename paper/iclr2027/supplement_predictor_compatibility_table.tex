...
State & Official checkout and preprocessing verified; its set-transition output does not materialize the required frozen global five-fold source-only vector and target-risk bridge & Blocked from the claim lock; no score reported. \\
...
scGPT & Package is installed, but import fails before model construction because the current torchtext binary cannot load (Windows error 127) & Bounded package attempt only; no model result is claimed. \\
biolord & Source-only adapter and pinned compatible checkpoint are not materialized in the current workspace & Compatibility pending; no score is claimed. \\
PerturbNet & Source-only adapter and pinned compatible checkpoint are not materialized in the current workspace & Compatibility pending; no score is claimed. \\
Scouter & Source-only adapter and pinned compatible checkpoint are not materialized in the current workspace & Compatibility pending; no score is claimed. \\
