...
Pairwise risk comparisons retain ties as an explicit ordering state rather than coercing them into a strict direction. Separately, exact/tolerance ties are excluded from the strict-sign Beta posterior used for the stable-direction diagnostic. A strict direction is declared stable when the 95\% equal-tail Beta(1,1) posterior credible interval excludes $0.5$, with at least eight non-tie Monte Carlo resampling outcomes supporting the strict comparison; otherwise the pair remains unresolved. This threshold diagnoses state-estimation stability and is not a minimum number of biological replicates.

We report the finite-sampling floor, the measurement-adjusted discrepancy, and a conservative joint-floor stress test separately. Final detectability intervals use the empirical variance of complete seed-level U-corrected contributions and a Student-$t$ reference distribution; label/member bootstrap intervals are retained only where explicitly marked as descriptive sensitivity analyses. The decision-link association is descriptive over 18 directed source--target surface rows; its dependence-aware sensitivity resamples three unordered context-pair blocks while preserving both directions and all metric rows. No target labels or target-derived features enter the source-frozen predictor.
...
\subsection{Why the finite-measurement estimand is not interchangeable with Kendall or generic MMD}
For each perturbation pair, $D_{\rm adj}$ compares the complete categorical
laws on $\{-1,0,+1\}$. For example, source and target laws
$(0.9,0,0.1)$ and $(0.6,0,0.4)$ have the same modal strict order, so a
deterministic Kendall comparison after collapsing both laws to their modal orders records no
inversion. Nevertheless, their pair-state discrepancy is $0.09$: the ordering
has become substantially less reliable in the target. This is the population
quantity that is absent from a point-ranking comparison. Conversely, a
generic expression-space MMD can detect distributional changes that preserve
every risk ordering. The exact identity with one half of squared MMD holds
only for the declared delta kernel on pair states and does not turn
$D_{\rm adj}$ into a generic expression two-sample statistic.

The U-corrected estimator is unbiased when complete measurement-seed blocks
are independent across seeds and identically distributed within each context.
The two pseudoreplicates and three predictor members inside a seed are dependent
subunits and are aggregated before variance estimation; shared controls remain
inside the same seed block. The frozen
predictor and perturbation panel do not depend on target outcomes, and the
two contexts are evaluated with the declared resampling design. At the null
$\pi_{a,e_1}=\pi_{a,e_2}$, its first-order influence vanishes and the
order-2 statistic is degenerate. Ordinary percentile bootstrap intervals are
therefore not used to decide whether transport is detectable. The calibrated
procedure treats the 30 seed-level U-corrected contributions as independent
units, estimates their variance directly, and uses a Student-$t$ interval for
their mean. A complete-seed block bootstrap is retained as a sensitivity
analysis; the two context arms are never resampled independently.
...
\input{supplement_nadig_table.tex}

\subsection{Second raw-cell replication: Tian iPSC to day-7 neuron}

We additionally rebuilt the full source-frozen, cell-resampled hierarchy in
the Tian CRISPR screens~\citep{tian2019crispri}. Eligibility was frozen to the 26 single-gene
perturbations represented by at least 80 cells in both systems. For each
source direction, the 1,536-gene evaluation panel was chosen by variance in
source control cells while excluding perturbation targets. Source effects
were fitted on a disjoint half of source cells; evaluation then used 40 cells
per perturbation and control in each of 30 seeds with two independent
measurement replicates. Target cells were absent from panel selection and
prediction construction. Table~\ref{tab:tian-rawcell} reports the calibrated
Student-$t$ intervals over per-seed U-corrected contributions. Five of six
intervals exclude zero; the iPSC-to-neuron Systema row does not. This is a
complete raw-cell replication of the estimand and measurement floor, not an
aggregate rank-distance audit.

\input{supplement_tian_rawcell_table.tex}
...
\input{supplement_decision_robustness_table.tex}
...
\input{supplement_transport_decomposition_table.tex}
\input{supplement_rank_comparator_table.tex}
...
depth and is interpreted accordingly. In addition, we computed a separate
...
\input{supplement_finite_measurement_table.tex}

\subsection{Transport-aware adaptive measurement allocation}
\label{app:adaptive}

We evaluated the practical measurement-design proposal on the held-out
Frangieh risk grid, starting from 10 cells per perturbation and comparing
uniform, random, lowest-cell-support-first, highest-pair-state-entropy and
transport-variance allocation. Acquisition scores were frozen at the pilot
stage; the full-depth vectors were used only as evaluation references. The
transport-variance policy reduced mean absolute error by only $1.28\%$ on
average relative to uniform allocation, with gains that varied by budget and
surface. Random allocation happened to yield a larger average reduction
($2.46\%$), but this is not an acquisition method and was not tuned or
promoted. Resolution rates were identical across policies at each budget
(0\% through 40 cells, 5.6\% at 80 cells and 33.3\% at 160 cells). Thus the
registered adaptive policy did not deliver a material or consistent cell
saving and is retained as a negative practical-utility result rather than
being moved into the main narrative. Supplementary Table~\ref{tab:v3-adaptive}
and the complete 450-row ledger at
\path{artifacts/manifests/adaptive_measurement_v1/adaptive_measurement_allocation_v1.csv}
record the comparison.

\input{supplement_v3_adaptive_table.tex}
...
\input{supplement_incremental_decision_value_table.tex}
...
\input{supplement_target_audit_table.tex}
...
prespecified comparison therefore retains this policy as a negative supplementary result
rather than a main-text contribution.

\input{supplement_regret_bound_critical_table.tex}

\section{Can reordering be anticipated from source-only information?}
\label{app:prospective}

This analysis asks a narrower prospective question than the transport study: can a source-only feature ladder anticipate future reordering burden? The prespecified feature sets are cumulative uncertainty ($U$), geometry ($U+G$), support ($U+G+S$), and novelty ($U+G+S+N$). Target outcomes, target risk, and the measurement-adjusted transport value are unavailable to the predictor and excluded from the design matrix. The source-only task-specific usable universe contains approximately 28--44 labels; the 243-label universe belongs to the full-size shortlist surface.
...
\caption{Source-only prospective boundary. (A) The information boundary separates source-observable features from target outcomes reserved for evaluation. (B) Held-out Spearman correlation for continuous reordering burden. (C) Held-out AUPRC for high-burden classification. Each point is one transfer-by-metric task; thick marks summarize the median and interquartile range.}
...
\section{Strong comparator calibration}
\label{app:comparators-v3}

The finite-measurement claim was tested against the strongest registered
traditional alternatives, not only against an uncorrected rank statistic. The
benchmark contains two measurement laws, five depths, two resolution settings,
two noise scales and three inversion fractions, for 120 conditions and 120
trials per condition. Every method uses the same perturbation-label resamples.
The comparator set includes raw Kendall, Spearman and top-10 Jaccard, their
label-bootstrap versions, matched-null-subtracted Kendall and Jaccard, stable
inversion, and the U-corrected $D_{\rm adj}$. The null target is zero for the
measurement-adjusted quantities; the raw rank metrics are retained as
descriptive sampling-disagreement comparators rather than being relabelled as
transport estimands.

Raw Kendall, Spearman and Jaccard show high null false-transport rates because
they compare two finite orderings without removing the same-context sampling
component. Label bootstrap intervals quantify uncertainty around that
comparison but do not change its target. Matched-null subtraction restores
near-zero null false-transport for both Kendall and Jaccard and retains high
power on the registered non-zero inversion surfaces. $D_{\rm adj}$ has the
advantage of a single pair-state population interpretation and an explicit
order-2 finite-replicate correction; the simulation does not support a claim
that it universally dominates matched-null Kendall. Supplementary
Table~\ref{tab:v3-comparator} reports false-transport rate, coverage and power
side by side. The complete trial-level ledger is
\path{artifacts/manifests/strong_comparator_v3/strong_comparator_v3_trials.csv}.

\input{supplement_v3_comparator_table.tex}

\subsection{Real-data semi-synthetic calibration}
\label{app:realdata-semisynthetic}

The known-truth simulation is complemented by a real-data semi-synthetic
benchmark derived from the Frangieh raw-cell lineage. We used the canonical
Control-context raw-cell pseudoreplicate risk ledger, selected the first 60
labels in the metadata-only eligible intersection, and generated two
independent 12-replicate pseudo-contexts by resampling the observed
replicate pool at 10, 20, 40, 80 and 160 cells. The same-context arm has a
known transport null ($D_{\rm adj}=0$). For the controlled alternatives, a
single cyclic permutation of the first 5\%, 10\%, 20\% or 40\% of the frozen
label panel was applied to the target arm. This permutation was fixed before
any estimates were inspected and is a response-order stress test, not a
claim of a new biological replicate.

The original hierarchical percentile interval resampled the two arms
independently. On this discrete ledger it produced a mean null
false-transport rate of $0.548$ and coverage of $0.452$, exposing a
nonregular boundary failure rather than a biological transport signal. We
therefore replaced it with the inference specified in
Sec.~\ref{app:estimators}: complete measurement-seed blocks are the independent
units and the interval is based on the empirical variance of their
U-corrected contributions.

The replacement was evaluated in 2,880 known-truth null trials and 3,000
Frangieh semi-synthetic null trials. The known-truth false-transport rate was
$0.040$ and null coverage was $0.960$; the corresponding real-data
semi-synthetic values were $0.015$ and $0.985$. Thus the replacement is mildly
conservative, particularly on the discrete empirical ledger, rather than
anti-conservative at the no-transport boundary. Detection power was $1.00$ at
the registered 10\% and 40\% simulated inversion fractions; in the harder
Frangieh semi-synthetic benchmark it was $0.162$ and $0.717$, respectively.
Supplementary
Fig.~\ref{fig:dadj-calibration} and Table~\ref{tab:dadj-calibration} compare
the failed interval and calibrated replacement. The old result remains in the
audit ledger but is not used for any detectability conclusion.

\input{supplement_v3_realdata_semisynthetic_table.tex}

\begin{figure*}[t]
\centering
\includegraphics[width=0.96\textwidth]{../../results/figures/reliability_transportability/dadj_inference_calibration.pdf}
\caption{Calibration of finite-measurement $D_{\rm adj}$ inference. (A) Null
false-transport rate and (B) null coverage for the original percentile
interval and the seed-block variance replacement; error bars are 90\% Wilson
intervals over trials and dashed lines mark nominal levels. (C) Detection
power under controlled reordering alternatives.}
\label{fig:dadj-calibration}
\end{figure*}

\input{supplement_dadj_calibration_table.tex}
...
\input{supplement_external_gears_table.tex}

We separately executed the stricter modern-predictor bridge required for the
Frangieh claim lock. The official GEARS implementation was trained for 20
epochs in five label-disjoint folds independently within each source context.
Graph construction, validation and fitting used source cells only. Thirteen
labels absent from the official interaction graph were removed before
splitting, leaving 230 perturbations and 8,229 evaluation genes. The frozen
out-of-fold vector was then passed through the same 30-seed,
two-pseudoreplicate raw-cell bridge as the other predictor families, producing
the complete 18-row directed transfer--metric surface.
TxPert was treated under the same information gate: the available checkpoint
was trained on K562 rather than a source-matched Frangieh context, and using
target basal expression would violate the frozen-source contract. It is
therefore not included in the predictor-family concordance. These are
deliberate contract outcomes, not missing values imputed from a different
experiment; the machine-readable record is
\path{modern_families/txpert_information_contract.json}.

An earlier one-epoch Frangieh GEARS pilot is retained only as engineering
provenance and is not pooled as an additional replicate. The public-cohort
GEARS audits above answer a different response-prediction question and are
likewise not pooled with the strict Frangieh transport surface.
...
\input{supplement_external_model_summary_table.tex}
...
\input{supplement_predictor_compatibility_table.tex}
...
parameterization, we evaluated a source-only RBF kernel-ridge family on the
...
\input{supplement_predictor_family_sensitivity_table.tex}
\input{supplement_predictor_family_canonical_v2_table.tex}

\begin{figure}[t]
\centering
\includegraphics[width=0.96\textwidth]{figures/supplementary/predictor_family_transport.pdf}
\caption{\textbf{Matched source-frozen predictor-family transport.} (A)
Circle, square and triangle glyphs show $D_{\rm adj}$ for delta cosine,
Systema centroid and absolute-effect rank; area and opacity encode magnitude.
Every family uses the same six directed transfers and raw-cell bridge. (B)
Within-metric profile Spearman correlation with the prespecified bilinear
reference. Positive sign agreement does not imply identical transport
geometry or predictor superiority.}
\label{fig:predictor-family-transport}
\end{figure}

\subsection{Neural source-only families and finite-measurement simulation}

We extended the source-only sensitivity without changing the canonical
bilinear-ridge estimand. Two additional families were fit on the frozen
Frangieh surface: a direct MLP and a latent-response MLP. In every outer fold,
the ten-dimensional input basis and, for the latent family, the 32-dimensional
response basis were fit on source training conditions only. The models used
the prespecified $256$--$512$ GELU architecture, dropout $0.1$, AdamW and a
source-only inner selection of weight decay. Across the three source contexts,
the macro source-side OOF fidelity was $0.363$ for the direct MLP and $0.366$
for the latent-response MLP, compared with $0.346$ for the bilinear reference
and $0.355$ for RBF. These are adequacy and profile diagnostics, not target-
informed model selection; the target outcomes were not loaded during fitting.

The resulting predictor-family profiles, including strict source-frozen GEARS,
use the same contexts, metrics and raw-cell bridge. Bilinear, RBF, direct-MLP
and latent-MLP use the 243-label eligible panel; GEARS uses the verified
230-label intersection supported by its official interaction graph. Each
family contributes 18 directed transfer--metric rows on its declared panel;
it is a sensitivity analysis, not a predictor-superiority claim. Supplementary
Fig.~\ref{fig:upgrade-v2} shows this profile together with the corrected
finite-measurement calibration: two generative laws, 960 registered
conditions (six depths, four resolution settings, four noise scales and five
inversion settings), and 500 trials per condition. Here the $D_{\rm adj}$
truth is the finite-depth population pair-state distance at the declared
resolution, while Kendall, Spearman and top-10 Jaccard use their corresponding
finite-depth population-mean ordering targets. Directional states use the
exact Beta posterior with the frozen minimum strict support of eight; realized
pair ties are retained as a third state. The simulation is a calibration and
recovery screen; it does not justify a universal superiority claim for
$D_{\rm adj}$ or for any predictor family. Full machine-readable outputs are
\path{predictor_families_v2/canonical_four_family_transport.csv},
\path{neural_family_transport_profiles.csv} and
\path{simulation_v21/finite_measurement_primary_v21.csv}. The former
\path{simulation_v2} artifact is retained only as provenance and is not
pooled with the corrected grid.

\begin{figure}[t]
\centering
\includegraphics[width=\textwidth]{../../results/figures/reliability_transportability/reliability_transportability_supp_fig5_upgrade_v2.pdf}
\caption{\textbf{Source-only family and finite-measurement sensitivity.} (A)
Mean absolute error of the corrected ordering estimate relative to the finite-
depth pair-state truth across cell depth for Gaussian-heteroscedastic and
Student-$t$ measurements. (B) Bias surface for the Gaussian primary slice;
the horizontal axis is cell depth and the vertical axis is the true inversion
fraction. (C) Source-side OOF fidelity for each source context and predictor
family. (D) Deterministic ordering profiles for the three fixed metrics across
the four source-only families. The neural family and simulation analyses are
descriptive and were not used to tune target transport or alter the canonical
main-text estimand.}
\label{fig:upgrade-v2}
\end{figure}

\input{supplement_finite_measurement_v21_table.tex}

\subsection{Outcome-blind Feng multi-donor biological replication audit}
\label{app:feng}

We audited the public multi-donor human-pluripotent-stem-cell CRISPRi screen
before inspecting any transport outcome~\citep{feng2026crispri}. Eligibility
was frozen from metadata support: 19 lines from 10 donors, with 135 targets
supported in every line at the primary threshold of 20 cells. The response
file is an aggregate target-by-line LFC table rather than raw cell-level
outcomes. We therefore froze an outcome-blind 1,000-gene panel from the file
schema and computed target-level RMS response risk, then compared target
rankings across same-donor/different-line and different-donor pairs.

The audit is informative but not a positive replication of the full Frangieh
$D_{\rm adj}$ hierarchy. Same-donor/different-line pairs had median Kendall
distance $0.220$, whereas different-donor pairs had $0.227$; the corresponding
Spearman distances were $0.124$ and $0.131$, and top-10 Jaccard distances were
$0.818$ and $0.700$. The contrasts are small and metric-dependent: cross-donor
distance was slightly higher for Kendall and Spearman but lower for top-10
Jaccard. We retain this as a neutral aggregate rank audit, not as a negative
biological claim or a predictor-risk replication. A within-line cell
resampling floor and hierarchical cell-level bootstrap could not be computed
from the released aggregate LFC file; those analyses remain an explicit
reopening requirement rather than an imputed result. Supplementary
Table~\ref{tab:v3-feng} and
\path{artifacts/manifests/external/feng_hipsci/feng_biological_replication_pairs.csv}
contain the complete pair ledger.

\input{supplement_v3_feng_table.tex}
...
\input{supplement_context_audit_table.tex}

\subsection{Outcome-blind scPerturb3 context audit}

As a separate provenance audit, we examined the local Srivatsan--Trapnell
scPerturb3 file using observation and variable metadata only. The three
candidate cell-line contexts (A549, MCF7 and K562) share 189 perturbations and
753 matched dose--time units per pair, with a median of at least 213 cells per
matched unit. However, all 52 plates are unique to one cell line, so every
cell is on a context-unique plate. This fails the prespecified
no-unique-plate-confounding requirement. The source-only predictor was
therefore not run on these contexts, and this audit is reported as a
provenance boundary rather than as a negative biological result. The complete
outcome-blind record is
\texttt{artifacts/manifests/sciplex3\_context\_replication\_audit\_v2.json};
the corresponding evidence and reopening requirements are listed in
\texttt{EXTERNAL\_DATA\_NEEDED\_V2.md}.
...
The scientific regeneration path starts from the canonical manifest tables, fixed splits and resampling settings, and rebuilds the four main figures, Supplementary Figs.~S1--S7, and the compact supplementary tables. Comparator, Feng and adaptive-allocation ledgers are generated from their corresponding scripts and are linked to the tables above; they do not alter the canonical main figures. The anonymous repository documents the executable commands, software versions, public accession records and data lineage. Large raw H5AD/NPZ inputs remain external to the manuscript tree; the reviewable CSV/JSON products expose the values used in figures and tables.

\paragraph{Code and data availability.}
The anonymized core release is available at
\url{https://anonymous.4open.science/r/PTL-B377/} and contains
