...
genes, contexts and metrics and is retained as a deterministic full-depth
profile because the neural runs do not replace the canonical cell-resampled
$D_{\rm adj}$ surface. Supplementary Fig.~\ref{fig:upgrade-v2} shows this
profile together with the pre-registered finite-measurement calibration slice:
two generative laws, 60 conditions, 500 trials per condition, six cell-depths,
five inversion fractions and an exact Beta direction posterior with the frozen
minimum strict support of eight. The simulation is a calibration and recovery
screen; it does not justify a universal superiority claim for $D_{\rm adj}$
or for any predictor family. Full machine-readable outputs are
\texttt{neural\_family\_transport\_profiles.csv} and
\texttt{simulation\_v2/finite\_measurement\_primary.csv}.
...
Mean absolute error of the corrected ordering estimate relative to the known
latent ordering law across cell depth for Gaussian-heteroscedastic and
