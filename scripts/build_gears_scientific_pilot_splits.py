...
def condition_components(label: object, is_control: object) -> list[str]:
    ...
    if str(is_control).lower() in {"true", "1", "yes"} or text.lower() in {"ctrl", "control"}:
        return []
    # Several public CRISPR screens encode guide replicate as ``GENE_1``.
    # It identifies a guide, not an additional perturbation gene.
...
def canonical_condition(label: object, is_control: object) -> str:
    """Canonical perturbation identity used for condition-disjoint grouping."""

    components = condition_components(label, is_control)
    return "_".join(components) if components else "ctrl"
...
def build_condition_disjoint_split(
    ...
    condition_keys = non_control.apply(
        lambda row: canonical_condition(row["perturbation_label"], row["is_control"]),
        axis=1,
    )
    conditions = np.asarray(sorted(condition_keys.unique()))
    if len(conditions) < 3:
    ...
    condition_by_row = signature.apply(
        lambda row: canonical_condition(row["perturbation_label"], row["is_control"]),
        axis=1,
    )
    train_mask = controls | condition_by_row.isin(train_conditions)
    val_mask = condition_by_row.isin(val_conditions)
    test_mask = condition_by_row.isin(test_conditions)
    ...
    for dataset in args.datasets:
        ...
        for seed in args.seeds:
            try:
                ...
                # Report an ineligible public cohort instead of silently
                # dropping it or stopping the full multi-cohort campaign.
                audit = {
                    "split_family": "random_split",
                    "split_protocol": "random_condition_holdout",
                    "perturbation_overlap": "not_applicable",
                    "track": "signature",
                    "dataset_scope": dataset,
                    "seed": int(seed),
                    "status": "ineligible",
                    ...
                    "train_test_condition_overlap": 0,
                    "validation_test_condition_overlap": 0,
                    "pilot_scope": "official_gears_scientific_pilot_condition_disjoint",
                }
