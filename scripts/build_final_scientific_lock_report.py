...
def run(root: Path = ROOT) -> Path:
    ...
    predictor_family = _load_json(manifests / "predictor_family_transport_comparison.json")
    ...
    figure_outputs = figures.get("outputs", {}) if isinstance(figures.get("outputs", {}), dict) else {}
    main_figure_keys = {"fig1", "fig2", "fig3", "fig4"}
    supplementary_figure_keys = {"supp_fig1", "supp_fig2", "supp_fig3", "supp_fig4"}
    main_figure_count = len(main_figure_keys.intersection(figure_outputs))
    supplementary_figure_count = len(supplementary_figure_keys.intersection(figure_outputs))
    ...
    rows = [
        ...
        ("Predictor-family sensitivity", predictor_family.get("status", "unavailable"), "Matched Frangieh source-only RBF sensitivity; descriptive profile concordance only, not equivalence or broad generalization."),
        ...
        ("Main figures 1–4 + Supplementary S1–S4", figures.get("status", "unavailable"), "The four main figures and four supplementary figures each have PNG/PDF/SVG/TIFF exports and a canonical source table."),
    ...
    checklist = [
        ...
        ("24", "four main figures generated", main_figure_count == 4),
        ("25", "four supplementary figures generated", supplementary_figure_count == 4),
