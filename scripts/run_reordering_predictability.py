...
def run(root: Path = ROOT) -> dict:
    ...
    # The Nadig Claim Lock is a within-dataset HepG2/Jurkat ordering analysis.
    # It does not materialize the cross-dataset source-only target-label bridge
    # required for a prospective score, so preserve that direction explicitly.
    blocked = []
    for source_dataset, target_dataset in (("FrangiehIzar2021_RNA", "NadigOConner2024"), ("NadigOConner2024", "FrangiehIzar2021_RNA")):
        for model in ("source_u_only", "linear_source_features"):
            blocked.append({
                ...
                "reason": "Nadig has within-dataset per-label ordering outcomes, but no shared cross-dataset source-only target-label bridge exists",
            })
