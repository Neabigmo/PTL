...
def _items_from_merged_risks(risks: pd.DataFrame, all_labels: list[str]) -> pd.DataFrame:
    """Reconstruct canonical per-item burdens on each complete global surface.

    Shard-local per-item burdens are not additive because they omit every
    cross-shard perturbation pair.  Member-level risks *are* additive rows, so
    the canonical item table must be rebuilt only after those rows have been
    merged across the full fixed label universe.
    """

    frame = risks.copy()
    label_order = {label: index for index, label in enumerate(all_labels)}
    frame["_label_order"] = frame["perturbation_label"].astype(str).map(label_order)
    rows: list[dict[str, object]] = []
    group_columns = [
        "source_environment_id", "left_target_environment_id", "right_target_environment_id",
        "metric", "split_seed", "cell_budget", "cell_budget_label", "cell_budget_order",
    ...
    for key, group in frame.groupby(group_columns, sort=False, observed=True):
        source, left, right, metric, seed, cell_budget, cell_budget_label, cell_budget_order = key
        group = group.sort_values(["measurement_replicate", "_label_order"], kind="stable")
        replicates = sorted(group["measurement_replicate"].astype(int).unique())
        labels = (
            group.loc[group["measurement_replicate"].astype(int).eq(replicates[0])]
            .sort_values("_label_order", kind="stable")["perturbation_label"].astype(str).tolist()
        )
        if len(replicates) < 2 or len(group) != len(replicates) * len(labels):
            raise ValueError(f"incomplete global risk grid for {key}")
        expected_labels = set(labels)
        for replicate in replicates:
            replicate_labels = set(group.loc[group["measurement_replicate"].astype(int).eq(replicate), "perturbation_label"].astype(str))
            if replicate_labels != expected_labels:
                raise ValueError(f"replicate label mismatch for {key}, replicate={replicate}")
        left_mean = np.stack([
            group.loc[group["measurement_replicate"].astype(int).eq(rep)].sort_values("_label_order", kind="stable")["left_risk"].to_numpy(dtype=float)
            for rep in replicates
        ])
        right_mean = np.stack([
            group.loc[group["measurement_replicate"].astype(int).eq(rep)].sort_values("_label_order", kind="stable")["right_risk"].to_numpy(dtype=float)
            for rep in replicates
        ])
        burden = perturbation_reordering_burden(left_mean, right_mean, block_size=min(512, len(labels)))
        ordering = summarize_fixed_predictor_ordering(left_mean, right_mean)
        expected_pairs = len(labels) * (len(labels) - 1) // 2
        if burden["n_items"] != len(labels) or burden["n_pairs"] != expected_pairs:
            raise AssertionError(f"global burden dimensions disagree for {key}")
        reconstruction_error = abs(
            float(np.mean(burden["identifiable_burden"]))
            - float(ordering["measurement_corrected_ordering_divergence"])
        )
        if reconstruction_error >= 1e-12:
            raise AssertionError(f"global item mean does not reconstruct canonical D_adj for {key}")
        for index, label in enumerate(labels):
            rows.append({
                "source_environment_id": str(source),
                "left_target_environment_id": str(left),
                "right_target_environment_id": str(right),
                "metric": str(metric),
                "split_seed": int(seed),
                "perturbation_label": label,
                "cell_budget": int(cell_budget),
                "cell_budget_label": str(cell_budget_label),
                "cell_budget_order": int(cell_budget_order),
                "universe_mode": "matched_fixed",
                "effective_cell_budget": int(cell_budget),
                "cross_disagreement": float(burden["cross_burden"][index]),
                "within_disagreement_left": float(burden["within_burden_left"][index]),
                "within_disagreement_right": float(burden["within_burden_right"][index]),
                "identifiable_divergence": float(burden["identifiable_burden"][index]),
                "stable_pair_fraction": float(ordering["stable_fraction_both"]),
                "n_items": int(len(labels)),
                "n_pairs": int(expected_pairs),
                "global_reconstruction_error": float(reconstruction_error),
                "truth_contract": "QC-passed Frangieh raw cells; source-frozen prediction; global merged fixed-label surface; independent A/B pseudoreplicates",
            })
    output = pd.DataFrame(rows)
    if output.empty:
        raise ValueError("no canonical global item rows were reconstructed")
    return output
...
def run(root: Path = ROOT, *, parts_root: Path | None = None, draws: int = 2000) -> dict[str, object]:
    ...
    _fixed_shard_items, _fixed_part_decisions, fixed_coverage, fixed_risks = _read_fixed_parts(parts_root, all_labels, fixed)
    ...
    # Recompute all pairwise burdens from the complete merged risk vectors.
    # Concatenating shard-local item burdens silently drops cross-shard pairs.
    items = _items_from_merged_risks(risks, all_labels)
    ...
    report = {
        ...
        "checks": {
            ...
            "canonical_items_recomputed_from_global_merged_risks": True,
            "max_item_mean_reconstruction_error": float(items["global_reconstruction_error"].max()),
        },
    }
