"""Generate the auditable panel-to-artifact map for the submission figures."""
...
def write_data_map(root: Path) -> Path:
    path = root / "paper/iclr2027/FIGURE_DATA_MAP.md"
    lines = ["# Figure data map", "", "Every submission panel is traceable to a canonical artifact and filter below.", "", "| Figure | Panel | Source artifact | Columns / fields | Filtering | Aggregation |", "|---|---|---|---|---|---|"]
    lines.extend("| " + " | ".join(row) + " |" for row in ROWS)
    lines += ["", "Repository-only auxiliary analyses (condition holdout, failure anatomy, and the former broad deployment gate) are not manuscript figures. Global contracts: metric colors are fixed; unavailable cells remain NA/gray; strict states require support ≥8; no target-informed feature enters Supplementary Fig. S1; no mock numerical values are used."]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
    return path


__all__ = ["write_data_map"]
