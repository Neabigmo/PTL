...
STATE_DEFINITION = "tie-aware 95% equal-tail Beta(1,1) posterior credible interval excludes 0.5 with minimum strict support >=8; ties excluded; risk p-q"
...
def _pair_records(source: str, target: str, metric: str, group: pd.DataFrame, seeds: list[int]) -> pd.DataFrame:
    ...
    for p, q in itertools.combinations(labels, 2):
        ...
        # Persist the empirical direction counts as well as the posterior
        # probabilities.  The posterior is appropriate for stable-state
        # classification, but it is not an order-law probability and cannot
        # support the exact tie/strength/flip decomposition by itself.
        source_lower_count = int(np.sum(source_difference < -1e-8))
        source_higher_count = int(np.sum(source_difference > 1e-8))
        target_lower_count = int(np.sum(target_difference < -1e-8))
        target_higher_count = int(np.sum(target_difference > 1e-8))
        ...
        rows.append({
            ...
            "source_lower_count": source_lower_count, "source_higher_count": source_higher_count,
            "target_lower_count": target_lower_count, "target_higher_count": target_higher_count,
