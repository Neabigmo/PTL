...
def parse_args() -> argparse.Namespace:
    ...
    run_one.add_argument(
        "--gene-panel",
        default="",
        help="Optional newline-delimited gene panel. Used only for a declared shared-gene comparison.",
    )
...
def prepare_split_data(
    ...
    split_payload = read_json(split_json_path)
    train_frame, val_frame, test_frame, gene_columns = materialize_run_data(
        repository,
        split_payload,
...
def run_single(
    ...
    if gene_panel is not None:
        gene_subset = [line.strip() for line in gene_panel.read_text(encoding="utf-8").splitlines() if line.strip()]
        if not gene_subset:
            raise ValueError(f"gene panel is empty: {gene_panel}")
        gene_space_policy = f"explicit_panel_{len(gene_subset)}"
        expected_gene_count = len(gene_subset)
    else:
        gene_space_policy = (
    ...
    prepared_data = prepare_split_data(repository, split_json, gene_subset=gene_subset)
    execute_run(repository=repository, run_spec=run_spec, force=True, prepared_data=prepared_data)
