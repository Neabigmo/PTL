...
The formal Claim Lock uses continuous rank displacement as one robustness
metric.  The theoretical measurement-corrected decomposition is a different,
...
def stable_order_mask(
    ...
    *,
    ...
    """Return stable mask, posterior interval, and pair indices."""

    values = _validate_replicates(risk_replicates, "risk_replicates")
    if not 0.0 < credible_level < 1.0:
        raise ValueError("credible_level must lie in (0, 1)")
    pairs, probabilities = pairwise_order_probabilities(values, tie_tolerance=tie_tolerance)
    successes = np.rint(probabilities[:, 2] * values.shape[0]).astype(int)
    trials = values.shape[0]
    tail = (1.0 - credible_level) / 2.0
    lower = beta.ppf(tail, successes + 1, trials - successes + 1)
    upper = beta.ppf(1.0 - tail, successes + 1, trials - successes + 1)
    stable = (lower > 0.5) | (upper < 0.5)
...
def summarize_fixed_predictor_ordering(
    ...
    *,
    ...
    corrected, rhs = measurement_corrected_ordering_divergence(pi_left, pi_right)
    stable_left, interval_left, _ = stable_order_mask(left, credible_level=credible_level, tie_tolerance=tie_tolerance)
    stable_right, interval_right, _ = stable_order_mask(right, credible_level=credible_level, tie_tolerance=tie_tolerance)
    stable_both = stable_left & stable_right
    left_sign = np.where(pi_left[:, 2] > 0.5, 1, np.where(pi_left[:, 2] < 0.5, -1, 0))
    right_sign = np.where(pi_right[:, 2] > 0.5, 1, np.where(pi_right[:, 2] < 0.5, -1, 0))
    return {
        ...
        "identity_absolute_error": float(abs(corrected - rhs)),
