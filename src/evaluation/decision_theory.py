...
def top_k_decision_metrics(
    ...
    *,
    ...
    # Keep the historic random-reference normalization for continuity, and
    # expose a second, bounded scale whose upper endpoint is the deterministic
    # worst k-item choice under the evaluated target outcome.  The two scales
    # answer different questions and must not be silently substituted.
    worst = deterministic_top_k(-target, k)
    worst_mean = float(np.mean(target[worst]))
    worst_denominator = worst_mean - oracle_mean
    worst_normalized = float(regret / worst_denominator) if worst_denominator > 0.0 else float("nan")
    ...
    return {
        ...
        "worst_target_risk": worst_mean,
        "regret": float(regret),
        "normalized_regret": normalized,
        "normalized_regret_denominator": float(denominator),
        "worst_oracle_normalized_regret": worst_normalized,
        "worst_oracle_normalized_regret_denominator": float(worst_denominator),
...
def replicate_decision_curve(
    ...
    *,
    ...
    for budget in budgets:
        ...
        numeric_keys = (
            "retention", "selected_target_risk", "oracle_target_risk", "random_target_risk", "worst_target_risk",
            "regret", "normalized_regret", "normalized_regret_denominator", "worst_oracle_normalized_regret",
            "worst_oracle_normalized_regret_denominator", "boundary_inversion",
