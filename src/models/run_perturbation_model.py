...
def materialize_official_gears_go_subgraph(
    ...
    *,
    ...
    if not source.is_file():
        _build_gears_go_subgraph_from_official_gene2go(
            data_path,
            source,
            relevant_genes,
        )
    ...
    if preserve_full_copy and not backup.is_file():
...
def _build_gears_go_subgraph_from_official_gene2go(
    data_path: Path,
    destination: Path,
    relevant_genes: set[str],
    *,
    max_neighbors: int = 21,
) -> None:
    """Build GEARS' thresholded Jaccard GO graph from its official gene2go file.

    The GEARS archive is a cache of this graph.  When the archive endpoint is
    unavailable, constructing the same thresholded Jaccard relation from the
    official ``gene2go_all.pkl`` keeps the prior source and graph definition
    intact.  The streamed output keeps only the 21 strongest incoming edges
    per gene—the exact downstream neighbourhood width used by GEARS—rather
    than materializing a multi-million-edge all-gene CSV for every run.
    """

    from scipy import sparse

    gene2go_path = data_path / "gene2go_all.pkl"
    if not gene2go_path.is_file():
        raise FileNotFoundError(f"missing official GEARS GO mapping: {gene2go_path}")
    with gene2go_path.open("rb") as handle:
        gene2go = pickle.load(handle)
    genes = sorted(str(gene) for gene in relevant_genes if str(gene) in gene2go and gene2go[str(gene)])
    if not genes:
        raise ValueError("none of the requested GEARS genes have official GO annotations")
    ...
    for row, gene in enumerate(genes):
        gene_terms = gene2go[gene]
        row_index.extend([row] * len(gene_terms))
        column_index.extend(term_index[term] for term in gene_terms)
    membership = sparse.csr_matrix(
        (np.ones(len(row_index), dtype=np.uint8), (row_index, column_index)),
        shape=(len(genes), len(terms)),
        dtype=np.uint8,
    )
    overlap = (membership @ membership.T).tocsr()
    degrees = np.asarray(membership.sum(axis=1)).ravel().astype(float)
    destination.parent.mkdir(parents=True, exist_ok=True)
    temporary = destination.with_suffix(destination.suffix + ".generated")
    with temporary.open("w", encoding="utf-8", newline="") as handle:
        writer = csv.writer(handle)
        writer.writerow(["source", "target", "importance"])
        for target in range(overlap.shape[0]):
            start, end = overlap.indptr[target], overlap.indptr[target + 1]
            neighbors = overlap.indices[start:end]
            shared = overlap.data[start:end].astype(float)
            if not len(neighbors):
                continue
            importance = shared / (degrees[target] + degrees[neighbors] - shared)
            keep = np.flatnonzero(importance > 0.1)
            if len(keep) > max_neighbors:
                keep = keep[np.argpartition(importance[keep], -max_neighbors)[-max_neighbors:]]
            for index in keep[np.argsort(-importance[keep], kind="stable")]:
                writer.writerow([genes[int(neighbors[index])], genes[target], f"{importance[index]:.8g}"])
    temporary.replace(destination)


def _link_or_copy(source: Path, destination: Path) -> None:
    """Stage immutable GEARS resources without multiplying their disk cost."""
...
def stage_official_gears_resources(data_path: Path) -> None:
    """Share official GEARS downloads across independent seed/run directories.

    The public graph archive is large, while each run subsequently replaces
    only its local graph *link* with a compact gene-subgraph.  This leaves the
    shared full resource immutable and prevents a 21-run campaign from making
    21 full copies before any training begins.
    """

    resource_root = ROOT / "data" / "external" / "gears_official_resources"
    ...
    _link_or_copy(resource_root / "gene2go_all.pkl", data_path / "gene2go_all.pkl")
    _link_or_copy(
        resource_root / "essential_all_data_pert_genes.pkl",
        data_path / "essential_all_data_pert_genes.pkl",
    )
...
def run_gears_adapter(row: pd.Series, args: argparse.Namespace, command: str, registry_path: Path) -> None:
    ...
    try:
        ...
        data_path.mkdir(parents=True, exist_ok=True)
        stage_official_gears_resources(data_path)
        # Public cohorts contain valid perturbation genes that are absent from
        # GEARS' packaged ``essential_all`` node list.  Build the perturbation
        # graph over the explicit, source-derived pilot gene universe instead:
        # the GO prior remains official, while every held-out perturbation
        # identity has an addressable graph node.
        ...
        # GEARS' GeneSimNetwork is indexed by ``pert_list`` (perturbation
        # nodes), not by the full expression panel.  Restricting the GO graph
        # to active perturbation genes keeps every edge addressable by that
        # node map while expression modelling still uses the selected genes.
        relevant_genes = active_perturbation_genes
        materialize_official_gears_go_subgraph(
            data_path,
            relevant_genes,
            preserve_full_copy=False,
        )
        ...
        model_details = {
            ...
            "graph_adapter": "official_gears_gene2go_on_explicit_pilot_perturbation_graph_and_coexpression",
