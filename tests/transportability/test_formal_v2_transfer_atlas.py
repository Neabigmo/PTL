...
def test_formal_transfer_atlas_has_two_complete_off_diagonal_surfaces() -> None:
    ...
    for baseline, group in frame.groupby("baseline"):
        assert group[["source_environment_id", "target_environment_id"]].drop_duplicates().shape[0] == 56
        assert group["same_source_target"].sum() == 0
        assert not (group["source_environment_id"] == group["target_environment_id"]).any()
